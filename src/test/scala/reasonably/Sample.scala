...
class ReasonablySampleSpec extends FunSpec with Matchers {
  import Algebra._
  import Tester._
  ...
  val interpreterTest = TestErrorInterpreter.or[({type f[x]=Coproduct[Auth,Interact,x]})#f](
    TestAuthenticator or TestConsole)
  ...
  describe("A program") {
    it("should login and authorize") {
      ...
      result shouldBe (List("OK", "Hello john.snow!", "UUDDLRLRBA"), Right(true))
    }
    ...
    it("should login and not authorize") {
      ...
      result shouldBe (List("OK", "Hello john.rain!", "Go away!"), Right(false))
    }
    ...
    it("should not login if password is wrong") {
      ...
      result shouldBe (List("OK", "Wrong ID or password!", "Go away!"), Right(false))
    }
    ...
    it("should throw error when user ID is empty") {
      ...
      result._2.left.get.getMessage shouldBe "User ID is empty"
    }
  }
}
